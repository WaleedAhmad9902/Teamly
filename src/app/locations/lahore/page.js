...
const LahorePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/data/lahoredata.json'); // Adjust the path as needed
  ...
  return (
    <div>
      ...
      >
        <h1 className="text-4xl font-bold mb-4">Lahore</h1>
        <button 
      ...
      <div className="bg-white flex items-center justify-center p-8">
        <p className="text-lg text-gray-800 text-center">
          Daftarkhwan has 6 vibrant sites with easy commutes across Lahore. From the commercial hub of Gulberg to the liveliness of DHA. Join a community of ambitious entrepreneurs in Punjab's main business districts.
        </p>
      </div>
      ...
      {selectedWorkspace && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
            ...
            <button
              className="w-full bg-gray-500-500 hover:bg-orange-600 text-white py-2 rounded transition duration-200"
